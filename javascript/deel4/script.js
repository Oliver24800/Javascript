...
var part4 = {
    ...
    instruction1: function () {
        ...
        var student = {
            naam : "Yosef East",
            group : "MD2",
            lessons : ["FRO", "BAP", "PROJ"]
        };

        // Zorg ervoor dat de opdracht goed wordt uitgevoerd en dat de content wordt bijgewerkt.
        var result = "Student " + student.naam + " zit in groep " + student.group + " en zijn eerste les is " + student.lessons[0];
    ...
    instruction2: function () {
        ...
        for (var i = 0; i < studentsArray.length; i++) {
            var test = studentsArray[i].split(",");
            //result += '{"name":"' + test[0] + '","group":"' + test[1] + '"}, ';
            var r = {
                name: test[0],
                group: test[1],
            };
            result.push(r);
            //result +=`{"name":"${test[0]","group":"${test[0]}"}, `;
        }
